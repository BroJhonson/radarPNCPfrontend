...
export default function initRadarPage() {
    ...
    const formatDateTime = (dateTimeString) => {
        ...
        try {
            // Verifica se a string contém 'T', indicando data e hora
            if (dateTimeString.includes('T')) {
                const dateObj = new Date(dateTimeString);
                if (isNaN(dateObj.getTime())) return 'Data/Hora Inválida';
                return dateObj.toLocaleString('pt-BR', {
                    year: 'numeric', month: '2-digit', day: '2-digit',
                    hour: '2-digit', minute: '2-digit'
                });
            } else {
                // Se for apenas data (YYYY-MM-DD), adicionamos T00:00:00 para evitar problemas de fuso
                const dateObj = new Date(dateTimeString + 'T00:00:00');
                if (isNaN(dateObj.getTime())) return 'Data Inválida';
                // Exibimos apenas a data
    ...
    if (offcanvasFiltrosBody) {
        ...
        offcanvasFiltrosBody.addEventListener('click', function(event) {
            if (event.target.classList.contains('btn-limpar-grupo')) {
                ...
                // Se algum filtro de palavra-chave foi limpo, salvamos o novo estado
                if (estadoAlterado) {
    ...
    inputField.addEventListener('keyup', function(e) {
        if (e.key === 'Enter' || e.key === 'NumpadEnter' || e.keyCode === 13) {
            ...
            // NÃO USAMOS MAIS 'palavrasArray'. Em vez disso, decidimos
            // qual array global usar com base no 'tipo'.
            if (tipo === 'inclusao') {
    ...
    function renderLicitacoesTable(licitacoes) {
        ...
        licitacoes.forEach(lic => {
            ...
            tr.innerHTML = `
                ...
                <td data-label="Atualização" class="align-middle">${dataAtualizacaoDisplay}</td>
    ...
    async function inicializarPagina() {
        ...
        await popularStatus();
        // aplicarEstadoCollapse(); // Descomente se quiser reativar
